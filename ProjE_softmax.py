...
def main(_):
    ...
    softmax_log = open("softmax_log.txt", "w")
    softmax_log.write("")
    softmax_log.close()
    # open in append mode
    softmax_log = open("softmax_log.txt", "a")
    ...
    with tf.Session() as session:
        ...
        if args.load_model is not None and os.path.exists(args.load_model):
            ...
            softmax_log.write("Load model from %s, iteration %d restored." % (args.load_model, iter_offset))
            softmax_log.write("\n")
        ...
        for data_func, test_type in zip([model.validation_data, model.testing_data], ['VALID', 'TEST']):
            ...
            softmax_log.write("waiting for worker finishes their work")
            softmax_log.write("\n")
            evaluation_queue.join()
            softmax_log.write("all worker stopped.")
            softmax_log.write("\n")
        ...
        for n_iter in range(iter_offset, args.max_iter):
            ...
            softmax_log.write("initializing raw training data...")
            softmax_log.write("\n")
            nbatches_count = 0
            for dat in model.raw_training_data(batch_size=args.batch):
                raw_training_data_queue.put(dat)
                nbatches_count += 1
            softmax_log.write("raw training data initialized.")
            softmax_log.write("\n")
            ...
            softmax_log.write("")
            softmax_log.write("\n")
            softmax_log.write("iter %d avg loss %.5f, time %.3f" % (n_iter, accu_loss / ninst, timeit.default_timer() - start_time))
            softmax_log.write("\n")
            ...
            if n_iter % args.save_per == 0 or n_iter == args.max_iter - 1:
                ...
                softmax_log.write("Model saved at %s" % save_path)
                softmax_log.write("\n")
